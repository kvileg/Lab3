...
{\ttfamily websocket-\/extensions} provides a container for registering extension plugins, and provides all the functions required to negotiate which extensions to use during a session via the {\ttfamily Sec-\/\+Web\+Socket-\/\+Extensions} header. By implementing the APIs defined in this document, an extension may be used by any Web\+Socket library based on this framework.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29842}{}\doxysection{\texorpdfstring{Installation}{Installation}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29842}

\begin{DoxyCode}{0}
\DoxyCodeLine{\$\ npm\ install\ websocket-\/extensions}

\end{DoxyCode}
\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29843}{}\doxysection{\texorpdfstring{Usage}{Usage}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29843}
...
The library is designed with the aim that any protocol implementation and any extension can be used together, so long as they support the same abstract representation of frames and messages.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29844}{}\doxysubsection{\texorpdfstring{Data types}{Data types}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29844}
The APIs provided by the framework rely on two data types; extensions will expect to be given data and to be able to return data in these formats\+:\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29845}{}\doxysubsubsection{\texorpdfstring{$<$em$>$\+Frame$<$/em$>$}{<em>Frame</em>}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29845}
...
\end{longtabu}
\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29846}{}\doxysubsubsection{\texorpdfstring{$<$em$>$\+Message$<$/em$>$}{<em>Message</em>}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29846}
...
\end{longtabu}
\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29847}{}\doxysubsection{\texorpdfstring{For driver authors}{For driver authors}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29847}
...
In the following examples, {\ttfamily exts} refers to this {\ttfamily Extensions} instance.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29848}{}\doxysubsubsection{\texorpdfstring{Client sessions}{Client sessions}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29848}
...
If the server has sent any extension responses that the client does not recognize, or are in conflict with one another for use of RSV bits, or that use invalid parameters for the named extensions, then {\ttfamily exts.\+activate()} will {\ttfamily throw}. In this event, the client driver should fail the connection with closing code {\ttfamily 1010}.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29849}{}\doxysubsubsection{\texorpdfstring{Server sessions}{Server sessions}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29849}
...
Calling {\ttfamily exts.\+generate\+Response(header)} activates those extensions the client has asked to use, if they are registered, asks each extension for a set of response parameters, and returns a string containing the response parameters for all accepted extensions.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29850}{}\doxysubsubsection{\texorpdfstring{In both directions}{In both directions}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29850}
...
The callback is invoked when all extensions have finished processing any messages in the pipeline and it\textquotesingle{}s safe to close the socket.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29851}{}\doxysubsection{\texorpdfstring{For extension authors}{For extension authors}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29851}
...
The extension must decide which set of parameters it wants to accept, if any, and return a {\itshape Server\+Session} if it wants to accept the parameters and {\ttfamily null} otherwise.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29852}{}\doxysubsubsection{\texorpdfstring{$<$em$>$\+Client\+Session$<$/em$>$}{<em>ClientSession</em>}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29852}
...
This returns the set of parameters the server session wants to send in its {\ttfamily Sec-\/\+Web\+Socket-\/\+Extensions} response header. Only one set of parameters is returned to the client per extension. Server sessions that would confict on their use of RSV bits are not activated.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29854}{}\doxysubsubsection{\texorpdfstring{$<$em$>$\+Session$<$/em$>$}{<em>Session</em>}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29854}
...
The framework will call this method when the Web\+Socket session ends, allowing the session to release any resources it\textquotesingle{}s using.\hypertarget{md_node__modules_2websocket-extensions_2README_autotoc_md29855}{}\doxysection{\texorpdfstring{Examples}{Examples}}\label{md_node__modules_2websocket-extensions_2README_autotoc_md29855}
